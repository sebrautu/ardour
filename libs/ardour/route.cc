...
Route::push_solo_upstream (int delta)
{
	...
	for (FedBy::iterator i = _fed_by.begin(); i != _fed_by.end(); ++i) {
		std::shared_ptr<Route> sr (i->r.lock());
		if (sr) {
	...
	*/
	...
	if (is_master ()) {
		...
		if (_volume_applies_to_output) {
			...
			_volume->activate ();
			main_outs()->add_gain (std::shared_ptr<GainControl> ());
		}
	}
...
bool
Route::add_fed_by (std::shared_ptr<Route> other, bool via_sends_only)
{
...
bool
Route::feeds (std::shared_ptr<Route> other, bool* via_sends_only)
{
	const FedBy& fed_by (other->fed_by());

	for (FedBy::const_iterator f = fed_by.begin(); f != fed_by.end(); ++f) {
		std::shared_ptr<Route> sr = f->r.lock();
...
bool
Route::direct_feeds_according_to_reality (std::shared_ptr<Route> other, bool* via_send_only)
{
...
bool
Route::feeds_according_to_graph (std::shared_ptr<Route> other)
{
...
Route::shift (timepos_t const & pos, timecnt_t const & distance)
{
	...
	if (_pannable) {
		...
		for (ControlSet::Controls::const_iterator ci = c.begin(); ci != c.end(); ++ci) {
			std::shared_ptr<AutomationControl> pc = std::dynamic_pointer_cast<AutomationControl> (ci->second);
			if (pc) {
				std::shared_ptr<AutomationList> al = pc->alist();
	...
	/* processor automation (incl. gain, trim,..) */
	{
		...
		for (ProcessorList::iterator i = _processors.begin (); i != _processors.end (); ++i) {
			...
			for (set<Evoral::Parameter>::const_iterator p = parameters.begin (); p != parameters.end (); ++p) {
				std::shared_ptr<AutomationControl> ac = (*i)->automation_control (*p);
				if (ac) {
...
Route::pan_width_control() const
{
	...
	if (mixbus() && _ch_pre) {
		//mono blend
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl>(_ch_pre->control(Evoral::Parameter(PluginAutomation, 0, 1)));
	}
#endif
	if (Profile->get_mixbus() || !_pannable || !panner()) {
		return std::shared_ptr<AutomationControl>();
...
{
#ifdef MIXBUS
	std::shared_ptr<PluginInsert> eq = _ch_eq;
	...
	return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (eq->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, port_number)));
#else
	return std::shared_ptr<AutomationControl>();
#endif
}
...
Route::eq_shape_controllable (uint32_t band) const
{
	...
	switch (band) {
		case 0:
			return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (eq->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 4))); // lo bell
			break;
		case 3:
			return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (eq->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 3))); // hi bell
			break;
		default:
			break;
	}
...
Route::eq_enable_controllable () const
{
	...
	return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (eq->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 1)));
#else
	return std::shared_ptr<AutomationControl>();
#endif
}
...
Route::filter_freq_controllable (bool hpf) const
{
	...
	if (hpf) {
#ifdef MIXBUS32C
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (eq->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 5))); // HPF freq
#else
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (eq->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 2)));
#endif
	} else {
#ifdef MIXBUS32C
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (eq->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 6))); // LPF freq
#else
		return std::shared_ptr<AutomationControl>();
#endif
	}
...
#ifdef MIXBUS
	if (_ch_pre) {
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (_ch_pre->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 0)));
	}
#endif
	return std::shared_ptr<AutomationControl>();
}
...
#ifdef MIXBUS
	if (is_master() && _ch_post) {
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (_ch_post->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 1)));
	}
#endif
	return std::shared_ptr<AutomationControl>();
}
...
#ifdef MIXBUS
	if (_ch_comp) {
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (_ch_comp->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 1)));
	}
#endif
	return std::shared_ptr<AutomationControl>();
}
...
#ifdef MIXBUS
	if (_ch_comp) {
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (_ch_comp->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 2)));
	}
#endif
	return std::shared_ptr<AutomationControl>();
}
...
#ifdef MIXBUS
	if (_ch_comp) {
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (_ch_comp->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 3)));
	}
#endif
	return std::shared_ptr<AutomationControl>();
}
...
#ifdef MIXBUS
	if (_ch_comp) {
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (_ch_comp->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 4)));
	}
#endif
	return std::shared_ptr<AutomationControl>();
}
...
#ifdef MIXBUS
	if (_ch_comp) {
		return std::dynamic_pointer_cast<ARDOUR::AutomationControl> (_ch_comp->control (Evoral::Parameter (ARDOUR::PluginAutomation, 0, 5)));
	}
#endif
	return std::shared_ptr<AutomationControl>();
}
