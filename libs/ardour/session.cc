...
static void
trace_terminal (std::shared_ptr<Route> r1, std::shared_ptr<Route> rbase, bool sends_only)
{
	std::shared_ptr<Route> r2;
	...
	if (DEBUG_ENABLED(DEBUG::Graph)) {
		...
		for (RouteList::iterator i = rl->begin(); i != rl->end(); ++i) {
			...
			for (Route::FedBy::const_iterator f = fb.begin(); f != fb.end(); ++f) {
				std::shared_ptr<Route> sf = f->r.lock();
				if (sf) {
...
Session::resort_routes_using (std::shared_ptr<RouteList> r)
{
	...
	*/
	...
	std::shared_ptr<RouteList> sorted_routes = topological_sort (r, edges);
