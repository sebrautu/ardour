...
Editor::insert_source_list_selection (float times)
{
	...
	if (should_ripple()) {
		do_ripple (playlist, get_preferred_edit_position(), region->length() * times, std::shared_ptr<Region>(), true);
	} else {
...
Editor::crop_region_to (timepos_t const & start, timepos_t const & end)
{
	...
	for (vector<std::shared_ptr<Playlist> >::iterator i = playlists.begin(); i != playlists.end(); ++i) {
		...
		std::shared_ptr<Region> region_at_start = (*i)->top_region_at(start);
		std::shared_ptr<Region> region_at_end = (*i)->top_region_at(end);
		...
		/* now adjust lengths */
		for (vector<std::shared_ptr<Region> >::iterator i = regions.begin(); i != regions.end(); ++i) {
...
Editor::apply_filter (Filter& filter, string command, ProgressReporter* progress)
{
	...
	for (RegionSelection::iterator r = rs.begin(); r != rs.end(); ) {
		...
		AudioRegionView* const arv = dynamic_cast<AudioRegionView*>(*r);
		if (arv) {
			std::shared_ptr<Playlist> playlist = arv->region()->playlist();
